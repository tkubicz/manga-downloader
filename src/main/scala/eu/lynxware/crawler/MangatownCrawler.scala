...
class MangatownCrawler(mangaName: String) extends LazyLogging {
  ...
  private val listOfChaptersPattern = s"""(http://www.mangatown.com/manga/berserk/c(\\d{1,4}|\\d{1,4}.\\d{1,4})/)""".r
  private val listOfPartsInChapterPattern = s"(http:\\/\\/www\\.mangatown\\.com\\/manga\\/$mangaName\\/c\\d{1,4}\\/\\d{1,3}.html)".r
